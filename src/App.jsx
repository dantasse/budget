...
export default function App() {
  ...
  // the split op (the Reports split editor): create new categories and move
  // whole transactions onto them. parts: [{ name,
  // placement, txKeys }]; placement 'child' puts the part under the source,
  // 'sibling' beside it. Unassigned transactions stay on the source — no
  // forced "Other" part, no route (future transactions stay on the source too).
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', padding: '24px', display: 'flex', flexDirection: 'column', height: '100vh', boxSizing: 'border-box', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
        ...
        {activeTab === 'Reports'      && <ReportsTab      key={`${selectedBudgetId}_${activeScenario}`} rows={rows} loading={loading} budgetId={selectedBudgetId} scenario={activeScenario} catTree={catTree} catOptions={catOptions} mergeChildren={mergeChildren} onUpdateCategory={updateCategory} onBulkUpdateCategory={bulkUpdateCategory} onUpdateMemo={updateMemo} isMainScenario={activeScenario === MAIN} opsEnabled={activeScenario !== MAIN || editLiveData} onRenameNode={renameNode} onMoveNode={moveNode} onMergeNode={mergeNode} onUnmergeNode={unmergeNode} onAddParts={addCategoryParts} onAbsorbChildren={absorbChildren} onPushUndo={pushUndo} onRemoveUndos={removeUndos} />}
      </div>
    </div>
  )
}
